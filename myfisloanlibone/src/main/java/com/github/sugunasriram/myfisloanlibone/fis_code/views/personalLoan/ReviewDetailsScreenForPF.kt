...
@Composable
fun ReviewDetailsScreenForPF(navController: NavHostController,
                             purpose: String, fromFlow:
                             String) {
        ...
        if (isCompleted) {
            BottomSheetScaffold(
                ...
                FixedTopBottomScreen(
                    ...
                    onBackClick = {
                        goBackForPF(navController = navController, fromFlow = fromFlow)
                    },
...
fun goBackForPF(navController: NavHostController, fromFlow: String) {
...
fun PersonalDetailViewForPF(
    ...
    userDetails?.let { response ->
        ...
        response.data?.let { profile ->
            PersonalDetailsCardForPF(profile)
            OrderDetailTextForPF()
            EmploymentDetailsCardForPF(profile, purpose)
        }
    }
}

@Composable
fun PersonalDetailsCardForPF(profile: Profile) {
...
@Composable
fun EmploymentDetailsCardForPF(profile: Profile, purpose: String) {
    NumberFullWidthCard(cardColor = lightishGray) {
